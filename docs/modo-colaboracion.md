...
diferentes, PÁRAME y dímelo. Por ejemplo: "me estás pidiendo dos cosas no
relacionadas: (a) … y (b) …; sugiero terminar (a), commit, y luego (b) en una
sesión limpia. ¿De acuerdo?". Si una petición se sale de la fase actual, avísame
y propón apuntarla en `docs/` para no perderla.

La razón: mezclar temas llena tu contexto y nos hace perder el hilo. Ayúdame a
mantener una tarea por sesión.
...
corto en markdown: qué archivos tocarías, en qué orden, qué decisiones hay, y qué
podría salir mal. Espera mi OK. Si la tarea es grande, propón guardar el plan en
`docs/` y trabajar por trozos.

## 4. DAME FEEDBACK SOBRE CÓMO ESTAMOS PROGRAMANDO

No te limites a obedecer. De forma regular, y siempre que lo veas pertinente, dame
tu opinión técnica honesta. En concreto:
...
Soy técnico pero estoy aprendiendo desarrollo web, PWA y este flujo.

- Cuando uses un concepto o patrón nuevo, explícamelo en 2-3 frases sencillas.
- Si te pido algo que revela que no entiendo algo, no lo des por hecho:
  explícamelo en vez de solo arreglarlo.
...
- Pídeme el error completo si no lo tienes (traza, log de build), no adivines.
...
- Hacer commit cuando algo funciona (red de seguridad) y **pushear** (el
  contenedor web es efímero: sin push, se pierde).
- Empezar sesión/tarea limpia al cambiar de tema.
...
Cada cierto tiempo, en una sesión limpia, puedes pedirme explícitamente: "revisa
cómo estamos llevando el proyecto y dame feedback honesto: arquitectura,
seguridad, deuda técnica y hábitos de trabajo". Eso me da permiso para una
revisión amplia que normalmente no haría en mitad de una tarea concreta.
