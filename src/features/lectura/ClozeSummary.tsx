...
import type { ClozeStats } from './useClozeSession'

/** Fin de sesión de huecos: aciertos + XP/racha/nivel + logros (núcleo compartido). */
export function ClozeSummary({
  stats,
  onRestart,
  onExit,
}: {
  ...
  return (
    <Card title="¡Sesión completada! 🎉">
      {reviewed === 0 ? (
        <p>No hay huecos que repasar ahora mismo. Vuelve más tarde 👋</p>
      ) : (
        <>
          <p>
            Completados: <strong>{reviewed}</strong> · Acertados:{' '}
