...
export interface ClozeStats {
  reviewed: number
  recalled: number
  /** XP ganada en esta sesión (anclada a aciertos produciendo de memoria). */
  xpGained: number
...
/**
 * Sesión de "rellenar huecos": te mostramos el aforismo con una palabra tapada
 * y la escribes. Producción real → da XP, igual que el resto de la app. Reutiliza
 * el SRS y el progreso del núcleo (XP/racha/nivel/logros son transversales).
 * Mismo patrón que `useLecturaSession`, pero sobre huecos (`ClozeItem`).
 */
export function useClozeSession() {
  const [loading, setLoading] = useState(true)
  const [states, setStates] = useState<Map<string, SrsState>>(new Map())
  const [queue, setQueue] = useState<string[]>([])
  const [total, setTotal] = useState(0)
  const [stats, setStats] = useState<ClozeStats>({
    reviewed: 0,
    recalled: 0,
    xpGained: 0,
    totalXp: 0,
  ...
  const build = useCallback(async () => {
    setLoading(true)
    const now = Date.now()
    const ids = CLOZE_ITEMS.map((it) => it.id)
  ...
  const current: ClozeItem | null = currentId
    ? clozeItemById.get(currentId) ?? null
    : null
  ...
  const grade = useCallback(
    (g: Grade) => {
      ...
      // XP anclada al recuerdo real: escribir de memoria la palabra que falta.
      let gainedXp = 0
